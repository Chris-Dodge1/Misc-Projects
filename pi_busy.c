// Pi estimating program written in C, with busy-wait for enforcing mutual exclusion
#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
...
int main(int argc, char* argv[]) {
    ...
    // Move it allow thread 0 to enter first
    flag = 0;
...
void* thread_sum(void* rank) {
    ...
    for (i = my_first_i; i < my_last_i; i++, factor = -factor) {
        // Before entering the critical sectopm, execute the while loop
        // Will only exit if the thread before it increments the flag
        while (flag != my_rank);

        // Critical section; add to the global sum
        sum += factor / (2 * i + 1);

        // Increment the flag
        // Loop back to 0 if flag exceeds the highest rank
        flag++;
        flag = flag % thread_count;
    }
